...
import json
from SendDanMu import send_dm_long
from tuling import get_message
        ...
        if cmd == 'DANMU_MSG':
            ...
            try:
                if commentUser == "接待狗":
                    ...
                    if commentUser not in tuling_user_id.keys():
                        tuling_user_id[commentUser] = random.randint(1, 10000)
                    danmu = get_message(commentText, tuling_user_id[commentUser])
                    print(tuling_user_id[commentUser])
                    send_dm_long(danmu)
            except:
                pass
            return
...
try:
    ...
    try:
        ...
        loop.run_forever()
    loop.close()
    os.system("screen -dm python bilibiliClient.py")#自动重启
